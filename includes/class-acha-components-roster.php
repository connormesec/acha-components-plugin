...
class Acha_Components_Roster
{
    ...
    public function buildPlayerCardRoster()
    {
        ...
        if (isset($_GET['player']) && isset($_GET['team'])) {
            ...
            $team_id = $_GET['team'];
            $playerPage = new Acha_Components_Player_Stat_Page($player_id, $team_id, $styles);
            ...
            function createPlayerCard($player_id, $team_id, $headshot_image, $number, $name, $position, $hometown, $ht, $wt, $birthdate, $shoots, $last_team = null)
            {
                $player_card = '
            <div class="player_item clearfix" id="' . $player_id . '" data-team="'. $team_id .'">
            <div class="thumb">
        ...
        ';
            ...
            foreach ($roster_data->roster[0]->players as $player) {
                ...
                $content .= createPlayerCard($player->player_id, $roster_data->team_id, $headshot_image_link, $player->tp_jersey_number, $player->name, $player->position, $player->hometown, $player->height_hyphenated, $player->w, $player->birthdate, $player->shoots, $last_team);
            }
            //defense
        ...
        foreach ($rosters as $roster) {
            ...
            foreach ($roster->roster as $position) {
                foreach ($position->players as $player) {
                    ...
                    $content .= '<td id="' . $player->player_id . '">' . $player->player_id . '</td><td>';
                    if ($show_last_team) {
                    ...
                    $content .= '</td><td><button id="_' . $player->player_id . '">Save</button>';

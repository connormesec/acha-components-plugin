...
class Acha_Components_Player_Stat_Page
{
    ...
    public $season_id;
    public $style;
    function __construct($player_id, $team_id, $season_id, $style)
    {
    ...
    private function getAndCleanPlayerData($player_id, $team_id)
    {
        ...
        if (isset($roster_edits[$player_id])) {
            if (isset($roster_edits[$player_id]->last_team)) {
                $cleaned_up_player_info->last_team = $roster_edits[$player_id]->last_team;
            }
            if (isset($roster_edits[$player_id]->year_in_school)) {
                $cleaned_up_player_info->year_in_school = $roster_edits[$player_id]->year_in_school;
            }
        }
    ...
    private function makeAsyncCurlRequests($player_id, $team_id)
    {
        ...
        $ch_2 = curl_init("https://lscluster.hockeytech.com/feed/index.php?feed=statviewfeed&view=roster&team_id=" . $team_id . "&season_id=" . $this->season_id . "&key=e6867b36742a0c9d&client_code=acha&site_id=2&league_id=1&lang=en");
